...
def get_ordered_dict():
    tweets = []
    for line in open('2018-09-11-14-18-tweets-stream.txt', 'r'):
...
def calc_sentiment(texts, tweets_count):
    ...
    if(neg > pos):
        tweets_date.write("true" + ", " + "false" + "\n" )
    elif(pos > neg):
        tweets_date.write("false" + ", " + "true" +  "\n" )
    else:
        tweets_date.write("false" + ", " + "false" +  "\n" )
    # tweets_date.write(str("%.2f" % neg) + ", " +  str("%.2f" % neu) +  ", " + str("%.2f" % pos)  +  "\n" )
...
def calc_bitcoin_price(timestamp):
    ...
    if(close_price_variation > 0):
        close_price_up = "true"
        close_price_down = "false"
        close_price_stable = "false"
    elif(close_price_variation < 0):
        close_price_up = "false"
        close_price_down = "true"
        close_price_stable = "false"
    elif(close_price_variation == 0):
        close_price_up = "false"
        close_price_down = "false"
        close_price_stable = "true"

    # tweets_date.write("close_price_variation = " + "1" if close_price_variation > 0 elif  close_price_variation < 0 "-1" +  if ", "
    #  + "high_price_variation = " +  str("%.4f" % high_price_variation) +  ", "
    #  + "low_price_variation = " + str("%.4f" % low_price_variation)  +  ", "
    #  + "open_price_variation = " + str("%.4f" % open_price_variation) +  ", "
    #  + "volumeFrom_variation = " + str("%.4f" % volumeFrom_variation) + ", "
    ...
    tweets_date.write(close_price_up + ", " +  close_price_down + ", " + close_price_stable + ", ")


tweets_date = open('2018-09-11-tweets.csv', 'w')
